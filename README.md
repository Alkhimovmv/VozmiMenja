...
`deploy-local.sh update` делает бэкап, устанавливает зависимости, собирает backend/frontend, применяет безопасные seed-миграции блога, пересобирает sitemap и перезапускает сервисы, но не обновляет код из Git.
