...
# Не останавливаться при ошибках (будем обрабатывать их вручную)
# set -e отключен для лучшей обработки ошибок
...
if ping -c 1 8.8.8.8 >/dev/null 2>&1; then
    ...
    echo "Проверьте сетевые настройки и попробуйте снова"
    exit 1
fi
...
# Проверка и освобождение блокировки apt
echo "Проверка блокировок apt..."
if [ -f /var/lib/dpkg/lock-frontend ]; then
    print_warning "Обнаружена блокировка apt, ожидание..."
    # Ждем завершения других процессов apt
    i=0
    while fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1; do
        sleep 1
        i=$((i+1))
        if [ $i -gt 60 ]; then
            print_warning "Принудительное снятие блокировки apt..."
...
apt-get update -qq || {
    print_warning "apt-get update завершился с ошибкой, повторная попытка..."
    sleep 2
    apt-get update
}
